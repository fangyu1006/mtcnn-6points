...
sys.path.append('.')
sys.path.append('/home/iim/caffe/python')
    ...
    for box in rectangles:
        ...
        if np.max(Iou) < 0.3:
            ...
            if np.max(Iou) >= 0.65:
                ...
                f1.write("%s/positive/%s.jpg"%(image_size, p_idx) + ' 1 %.2f %.2f %.2f %.2f\n'%(offset_x1, offset_y1, offset_x2, offset_y2))
                cv2.imwrite(save_file, resized_im)
                #print(p_idx)
                #cv2.imshow(str(p_idx), resized_im)
                #cv2.waitKey(0)
                p_idx += 1

            elif np.max(Iou) >= 0.4:
                save_file = os.path.join(part_save_dir, "%s.jpg"%d_idx)
                f3.write("%s/part/%s.jpg"%(image_size, d_idx)     + ' -1 %.2f %.2f %.2f %.2f\n'%(offset_x1, offset_y1, offset_x2, offset_y2))
                cv2.imwrite(save_file, resized_im)
                d_idx += 1
f1.close()
f2.close()
